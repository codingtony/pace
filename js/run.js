...
App.init = function() {
	...
	$(".faster1").click(function() {
		var target = $(this).data('target');
		var update = $(this).data('update');
		eval("if (" + target + "  > 1) {" + target + "-=1;}");
		eval(update);
	});

	$(".faster2").click(function() {
		var target = $(this).data('target');
		var update = $(this).data('update');
		eval("if (" + target + "  > 10) {" + target + "-=10;}");
		eval(update);
	});
	...
	$("#inputCurrentCalibration").change(function() {
		var tmpVal = $(this).val();
		if (tmpVal.match(/^([0-9]\d*)$/)) {
			Cal.current = parseInt(tmpVal);
			Cal.calculate();
		}
...
Cal.calculate = function() {

	// update the footpod pace
	$("#footpacemetric").text(
			Run.convertSecsToStr(Run.convertPaceSpeed(Cal.footpod, "minkm",
					"minkm")));
	$("#footpaceimperial").text(
			Run.convertSecsToStr(Run.convertPaceSpeed(Cal.footpod, "minkm",
					"minmi")));
	$("#footspeedmetric").text(
			Run.convertPaceSpeed(Cal.footpod, "minkm", "kmh"));

	$("#footspeedimperial").text(
			Run.convertPaceSpeed(Cal.footpod, "minkm", "mph"));

	// update the threadmill pace
	$("#threadpacemetric").text(
			Run.convertSecsToStr(Run.convertPaceSpeed(Cal.threadmill, "minkm",
					"minkm")));
	$("#threadpaceimperial").text(
			Run.convertSecsToStr(Run.convertPaceSpeed(Cal.threadmill, "minkm",
					"minmi")));
	$("#threadspeedmetric").text(
			Run.convertPaceSpeed(Cal.threadmill, "minkm", "kmh"));

	$("#threadspeedimperial").text(
			Run.convertPaceSpeed(Cal.threadmill, "minkm", "mph"));

	var newCal = parseInt(Math.round(parseFloat((Cal.footpod / Cal.threadmill)
			* Cal.current)));

	$("#newCalibration").text(newCal);
...
App.calculate = function() {
	...
	$(".distance_unit>input[value=" + Run.distunit + "]").parent().addClass(
			"active");

	// update the pace
	$("#pacemetric").text(
			Run.convertSecsToStr(Run.convertPaceSpeed(Run.pacemetric, "minkm",
					"minkm")));
	$("#paceimperial").text(
			Run.convertSecsToStr(Run.convertPaceSpeed(Run.pacemetric, "minkm",
					"minmi")));
	$("#speedmetric")
			.text(Run.convertPaceSpeed(Run.pacemetric, "minkm", "kmh"));
	$("#speedimperial").text(
			Run.convertPaceSpeed(Run.pacemetric, "minkm", "mph"));
...
Run.convertSecsToStr = function(secs_) {
	...
	return h == 0 ? Run.pad(mm, 2) + ':' + Run.pad(ss, 2) : h + ':'
			+ Run.pad(mm, 2) + ':' + Run.pad(ss, 2);
};
